//   THIS IS NOT IN A WORKING STATE. ATTEMPTING TO SEPARATE BUFFER PARSE AND SEND INTO A SEPARATE FUNCTION
//   IT WOULD BE NICE TO BE ABLE TO FORK THE SEND BUT THEN THE SEND FUNCTION NEEDS CONTROL OF LORA,
//   WHICH I HAVE NOT FIGURED OUT HOW TO DO. Peripheral.take() HAS TO BE DONE IN ONE PLACE (MAIN?)
//   SO THEN LORA HAS TO BE PASSED ON SOMEHOW.
//
//   TRY JUST PARSE AND RETURN BUF TO BE SENT? THAT WILL NOT ALLOW FOR EVENTUALLY FORKING.
//
...
fn send_line(buffer: &Vec<u8, consts::U80>, lora ) -> () {
    // set this with
    ...
    loop {
        ...
        if good {
            ...
            if buffer.push(byte).is_err() || byte == 13 {
                ...
                send_line(&buffer, lora); 
                WOULD BE NICE TO BE ABLE TO FORK BUT SEND NEED LORA
                TRY JUST PARSE AND RETURN BUF TO SEND?
